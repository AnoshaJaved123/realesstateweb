    ...
    <div className='navbar relative z-10 flex flex-col md:flex-row md:justify-start justify-center items-center py-2 shadow-md sticky top-0  bg-white backdrop-filter backdrop-blur-lg bg-opacity-30 border-b border-gray-200'>
      ...
      <div className="nav">
        <ul className='flex items-center space-x-3 font-normal md:text-lg'>
          <Link href={'/'}><a className=" font-semibold hover:text-teal-700 text-teal-500" >HOME</a></Link>
          <Link href={'/agents'}><a className=" font-semibold hover:text-teal-700 text-teal-500" >AGENTS</a></Link>
          <Link href={'/services'}><a className=" font-semibold hover:text-teal-700 text-teal-500" >SERVICES</a></Link>
          <Link href={'/rent'}><a className=" font-semibold hover:text-teal-700 text-teal-500" >RENT</a></Link>
          <Link href={'/buy'}><a className=" font-semibold hover:text-teal-700 text-teal-500" >BUY</a></Link>
          <Link href={'/contact'}><a className=" font-semibold hover:text-teal-700 text-teal-500" >CONTACT</a></Link>
      ...
      <div className="cursor-pointer items-center cart absolute right-0 mx-4 flex">
        <span onMouseOver={() => { setdropdown(true) }} onMouseLeave={() => { setdropdown(false) }}>
          ...
          {user.value && <> <div className='flex flex-wrap'><p className='mx-2 my-1 p-1 text-teal-600'>{headername}</p>
