    ...
    <div className='navbar relative z-10 flex flex-col md:flex-row md:justify-start justify-center items-center py-2 shadow-md sticky top-0  bg-white backdrop-filter backdrop-blur-lg bg-opacity-30 border-b border-gray-200'>
      ...
      <div className="nav">
        <ul className='flex items-center space-x-3 font-normal md:text-lg'>
          <Link href={'/'}><a className=" font-semibold hover:text-stone-700 text-black" >HOME</a></Link>
          <Link href={'/agents'}><a className=" font-semibold hover:text-stone-700 text-black" >AGENTS</a></Link>
          <Link href={'/services'}><a className=" font-semibold hover:text-stone-700 text-black" >SERVICES</a></Link>
          <Link href={'/rent'}><a className=" font-semibold hover:text-stone-700 text-black" >RENT</a></Link>
          <Link href={'/buy'}><a className=" font-semibold hover:text-stone-700 text-black" >BUY</a></Link>
          <Link href={'/contact'}><a className=" font-semibold hover:text-stone-700 text-black" >CONTACT</a></Link>
      ...
      <div className="cursor-pointer items-center cart absolute right-0 mx-4 flex">
        <span onMouseOver={() => { setdropdown(true) }} onMouseLeave={() => { setdropdown(false) }}>
          ...
          {user.value && <> <div className='flex flex-wrap '><p className='mx-2 my-1 p-1 text-stone-700'>{headername}</p>
        ...
        {!user.value && <><Link href={'/loginadmin'} ><a><div className='flex mx-5 p-1'><GrUserAdmin className='mx-2' /><p className='text-sm text-stone-600'>Admin</p></div></a></Link>
          <Link href={'/login'} ><a>
            <button className=' flex mx-auto bg-transparent text-sm my-1 mx-2 hover:bg-stone-500 text-stone-700 font-semibold hover:text-white py-0 px-1 border border-stone-500 hover:border-transparent'>Login</button>
          </a></Link></>}
